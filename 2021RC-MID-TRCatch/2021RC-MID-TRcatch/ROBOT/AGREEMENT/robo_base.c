...
void CATCH_Init(ROBO_CATCH* Robo)       
{
//
//	if(catch_assist.state == 1 || catch_assist.state == 2 || catch_assist.state == 7 || catch_assist.state == 8)
//	
//	{
//       Speed_System* P_Speed=NULL;      //速度环信息和pid
//	     P_Speed=&Robo->SPEED_systemFinger; PID_Init(&P_Speed->Speed_PID,	10,	0,	0,	5000,	0,	5000,	2000); P_Speed->Motor_Num=0;
//	     P_Speed=&Robo->SPEED_systemLine; PID_Init(&P_Speed->Speed_PID,	10,	0,	0,	5000,	0,	5000,	2000); P_Speed->Motor_Num=1;
//	
//	}
//	else
//		{
			 Pos_System* P_Pos=NULL;  
				...
				P_Pos=&Robo->Pos_systemElbow; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	0,	1000);
				P_Pos->Motor_Num=2;		PID_Init(&P_Pos->Speed_PID,			17,	0,	0,	5000,	0,	0,	7000); 
				
				P_Pos=&Robo->Pos_systemWrist; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	5000,	800);
				P_Pos->Motor_Num=3;		PID_Init(&P_Pos->Speed_PID,			17,	0,	0,	5000,	0,	5000,	7000); 

				P_Pos=&Robo->Pos_systemLine; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	0,1000);
				P_Pos->Motor_Num=1;		PID_Init(&P_Pos->Speed_PID,			10,	0,	0,	5000,	0,	0,	5000); 
				
				P_Pos=&Robo->Pos_systemFinger; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	0,	1000);
				P_Pos->Motor_Num=0;		PID_Init(&P_Pos->Speed_PID,			10,	0,	0,	5000,	0,	0,	5000); 
//	}
...
//			{
//				Pos_System* P_Pos=NULL;  
//				P_Pos=&Robo->Pos_systemLine; PID_Init(&P_Pos->Pos_PID,			0,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=1;		PID_Init(&P_Pos->Speed_PID,			0,	0,	0,	5000,	0,	0,	7000); 
//				
//				P_Pos=&Robo->Pos_systemOpen; PID_Init(&P_Pos->Pos_PID,			1.5,	0,	0,	5000,	0,	5000,	5000);
//				P_Pos->Motor_Num=0;		PID_Init(&P_Pos->Speed_PID,			15,	0,	0,	5000,	0,	5000,	5000);
//				
//				
//				break;
//			}
...
//			{
//				Pos_System* P_Pos=NULL;  
//				P_Pos=&Robo->Pos_systemLine; PID_Init(&P_Pos->Pos_PID,			1.5,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=1;		PID_Init(&P_Pos->Speed_PID,			15,	0,	0,	5000,	0,	0,	7000); 
//				
//				P_Pos=&Robo->Pos_systemOpen; PID_Init(&P_Pos->Pos_PID,			0,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=0;		PID_Init(&P_Pos->Speed_PID,			0,	0,	0,	5000,	0,	0,	7000); 
//				break;
//			}
//		}
//	} 
...
//{
//	  Speed_System* P_Speed=NULL;      //速度环信息和pid
//		P_Speed=&Robo->SPEED_systemOpen; PID_Init(&P_Speed->Speed_PID,	10,	0,	0,	5000,	0,	5000,	5000); P_Speed->Motor_Num=0;
//		P_Speed=&Robo->SPEED_systemLine; PID_Init(&P_Speed->Speed_PID,	10,	0,	0,	5000,	0,	5000,	5000); P_Speed->Motor_Num=1;
//}else
//{
//        Pos_System* P_Pos=NULL;  
//	
//	      P_Pos=&Robo->Pos_system1; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	0,	3000);
//				P_Pos->Motor_Num=2;		PID_Init(&P_Pos->Speed_PID,			3,	0,	0,	5000,	0,	0,	5000); 
//				
//				P_Pos=&Robo->Pos_system2; PID_Init(&P_Pos->Pos_PID,			0.5,	0,	0,	5000,	0,	5000,	3000);
//				P_Pos->Motor_Num=3;		PID_Init(&P_Pos->Speed_PID,			3,	0,	0,	5000,	0,	5000,	5000); 
//	
//	      P_Pos=&Robo->Pos_systemLine; PID_Init(&P_Pos->Pos_PID,			1.5,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=1;		PID_Init(&P_Pos->Speed_PID,			15,	0,	0,	5000,	0,	0,	7000); 
//				
//				P_Pos=&Robo->Pos_systemOpen; PID_Init(&P_Pos->Pos_PID,			1.5,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=0;		PID_Init(&P_Pos->Speed_PID,			15,	0,	0,	5000,	0,	0,	7000); 
//	
//}
//if(state == 11 || state == 12 || state == 13 || state == 14|| state == 15 || state == 16 )
//{
//	      Pos_System* P_Pos=NULL;  
//	      P_Pos=&Robo->Pos_system1; PID_Init(&P_Pos->Pos_PID,			0,	0,	0,	5000,	0,	0,	7000);
//				P_Pos->Motor_Num=2;		PID_Init(&P_Pos->Speed_PID,			0,	0,	0,	5000,	0,	0,	7000); 
//				
//				P_Pos=&Robo->Pos_system2; PID_Init(&P_Pos->Pos_PID,			0,	0,	0,	5000,	0,	5000,	5000);
//				P_Pos->Motor_Num=3;		PID_Init(&P_Pos->Speed_PID,			0,	0,	0,	5000,	0,	5000,	5000); 
//}
}
...
//
//--------------------------------------------------------------------------------------------------//
//void Motor_Pos_Analysis(ROBO_CATCH* Robo,uint8_t* RX_Data,uint32_t Motor_Num)
//{
...
void PID_Send(ROBO_CATCH* Robo)
{
    if(catch_assist.flag)
		{
			Pos_System* P_Pos=NULL;
			P_Pos=&Robo->Pos_systemLine; PID_Pos_Cal(P_Pos,Robo->Tx_CAN2);
			P_Pos=&Robo->Pos_systemFinger; PID_Pos_Cal(P_Pos,Robo->Tx_CAN2);
			P_Pos=&Robo->Pos_systemElbow; PID_Pos_Cal(P_Pos,Robo->Tx_CAN2);
			P_Pos=&Robo->Pos_systemWrist; PID_Pos_Cal(P_Pos,Robo->Tx_CAN2);
			Send_To_Motor(&hcan2,Robo->Tx_CAN2);
		}
		else
		{
//			Speed_System* P_Speed=NULL;
//			P_Speed=&Robo->SPEED_systemFinger; PID_Speed_Cal(P_Speed,Robo->Tx_CAN1);
//			P_Speed=&Robo->SPEED_systemLine; PID_Speed_Cal(P_Speed,Robo->Tx_CAN1);
...
void PID_Pos_Cal(Pos_System* Pos_Motor,uint8_t *Tx_msg)       
{
	...
	if(Pos_Motor->Pos_PID.error > Pos_Motor->Pos_PID.error_max)//误差
	...
	Pos_Motor->Pos_PID.intergral = Pos_Motor->Pos_PID.intergral + Pos_Motor->Pos_PID.error;//积分
	...
	Pos_Motor->Pos_PID.derivative = Pos_Motor->Pos_PID.error - Pos_Motor->Pos_PID.error_last;//微分
	Pos_Motor->Pos_PID.error_last = Pos_Motor->Pos_PID.error;
	
	Pos_Motor->Pos_PID.output = Pos_Motor->Pos_PID.Kp*Pos_Motor->Pos_PID.error + Pos_Motor->Pos_PID.Ki*Pos_Motor->Pos_PID.intergral + Pos_Motor->Pos_PID.Kd*Pos_Motor->Pos_PID.derivative;//输出
	...
	//速度pid
		Pos_Motor->Speed_PID.error =  Pos_Motor->Pos_PID.output - Pos_Motor->Info.Speed;//误差	
...
//  switch(Motor_Num)
//  {
//    case 0x201:S_Motor=&Robo->SPEED_systemFinger;break;
//   case 0x202:S_Motor=&Robo->SPEED_systemLine;break;
...
void Catch_Pos_Analysis(ROBO_CATCH* Robo,uint8_t* RX_Data,uint32_t Motor_Num)
{
  ...
  switch(Motor_Num)
  {
    case 0x202:P_Motor=&Robo->Pos_systemLine;break;
		case 0x201:P_Motor=&Robo->Pos_systemFinger;break;
		case 0x203:P_Motor=&Robo->Pos_systemElbow;break;
		case 0x204:P_Motor=&Robo->Pos_systemWrist;break;
