...
void BASE_Init(ROBO_BASE *Robo)       
{
  ...
  P_Speed=&Robo->Speed_MotorRF; PID_Init(&P_Speed->Speed_PID,	7.0f,	0.1f,	0,	3000,	0,	3000,	5000); P_Speed->Motor_Num=2;
	P_Speed=&Robo->Speed_MotorRB; PID_Init(&P_Speed->Speed_PID,	0,	0,	0,	0,	0,	0,	0); P_Speed->Motor_Num=3;
}


void Motor_Speed_Analysis(ROBO_BASE* Robo,uint8_t* RX_Data)
{
  ...
  switch(Motor_Num)
  {
    ...
    case 0x203:S_Motor=&Robo->Speed_MotorRF;break;
		case 0x204:S_Motor=&Robo->Speed_MotorRB;break;  //抓捕
...
void PID_Send(ROBO_BASE* Robo)
{
  ...
  Send_To_Motor_(&hcan1,Robo->Tx_CAN1,Robo);
}
...
void Send_To_Motor_(CAN_HandleTypeDef *hcan,uint8_t* Tx_Data,ROBO_BASE* Robo)
{
